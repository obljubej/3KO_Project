...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(project_model_UPDATE_M) == (NULL)) &&
    !rtmGetStopRequested(project_model_UPDATE_M);
  while (runModel) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(project_model_UPDATE_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(project_model_UPDATE_M, true);
      }

      if (rtmGetStopRequested(project_model_UPDATE_M) == true) {
        rtmSetErrorStatus(project_model_UPDATE_M, "Simulation finished");
        break;
      }
    }

    project_model_UPDATE_step();

    /* Get model outputs here */
    rtExtModeCheckEndTrigger();
    stopRequested = !((rtmGetErrorStatus(project_model_UPDATE_M) == (NULL)) &&
                      !rtmGetStopRequested(project_model_UPDATE_M));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(project_model_UPDATE_M, "stopping the model");
  runModel = 0;
}
...
void *terminateTask(void *arg)
{
  ...
  /* Terminate model */
  project_model_UPDATE_terminate();
  rtExtModeShutdown(1);
  TgtConnTerm();
...
void *backgroundTask(void *arg)
{
  while (runModel) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(project_model_UPDATE_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(project_model_UPDATE_M, true);
      }
    }
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(project_model_UPDATE_M, 0);
  ...
  /* Initialize model */
  project_model_UPDATE_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(project_model_UPDATE_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(project_model_UPDATE_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(project_model_UPDATE_M, true);
    }
  }
